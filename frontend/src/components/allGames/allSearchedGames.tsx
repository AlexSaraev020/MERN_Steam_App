            ...
            <div className={`w-full flex items-center justify-center mt-10`}>
                <ul className={`w-11/12 sm:w-9/12 space-y-20 sm:space-y-8 bg-[#1e1e1e] rounded-xl flex flex-col justify-center items-center border-2 border-zinc-800 transition ease-in-out duration pb-10`}>
                    {slicedGames.map((game) => (
                        <Link to={`/game/${game.id}`} key={game.id} className={` shadow-glow-sm shadow-zinc-700 hover:shadow-${theme}-500 transition duration-500 delay-100 hover:scale-105 bg-neutral-900 p-2 sm:p-4 w-10/12 sm:w-11/12 flex flex-col sm:flex-row rounded-xl space-x-0 sm:space-x-3 mt-6 sm:mt-10 ease-in-out animate-fadeIn`}>
                            ...
                            <div className={`flex flex-col sm:flex-row justify-between w-full`}>
                                ...
                                <div className={`flex items-end justify-center sm:justify-end mt-4 sm:mt-0`}>
                                    <button className={`text-white bg-gradient-to-tr from-zinc-900 via-zinc-800 to-zinc-900 shadow-glow-sm shadow-${theme}-500 transition-all duration-500 focus:ring-4 focus:ring-${theme}-300 font-bold rounded-lg mt-2 xl:mt-0 text-md md:text-lg xl:text-xl px-2 py-1 md:px-5 md:py-2 focus:outline-none w-full sm:w-auto xl:w-48 xl:h-14`}>
                    ...
                    <div className={`w-10/12 sm:w-11/12 ${searchedGames.length > 10 ? 'block' : 'hidden'} flex py-4 space-x-6 justify-center`}>
                        {activePrevious && <button className={`text-${theme}-500 font-mono font-bold text-lg flex items-center justify-center backdrop-blur-lg bg-gradient-to-tr from-zinc-900 via-zinc-800 to-zinc-900 rounded-md py-2 px-6 shadow-md shadow-zinc-600 hover:shadow-glow hover:shadow-${theme}-500 hover:-translate-y-1 hover:scale-110 duration-700 w-28`} onClick={handlePrevious}>Previous</button>}
                        <div className={`flex justify-center space-x-3 text-lg font-semibold font-mono`}>
                            {activePrevPage && <h2 className={`text-${theme}-500 flex items-center justify-center px-6 border-2 rounded-lg border-zinc-700 bg-gradient-to-tr from-zinc-900 via-zinc-800 to-zinc-900 shadow-md shadow-zinc-600 transition-all duration-500 ease-in-out animate-fadeIn`}>{page}</h2>}
                            <h2 className={`text-${theme}-500 flex items-center justify-center px-6 border-2 rounded-lg border-zinc-700 bg-gradient-to-tr from-zinc-900 via-zinc-800 to-zinc-900 shadow-glow shadow-${theme}-500 scale-105 -translate-y-1 transition-all duration-500 ease-in-out animate-fadeIn`}>{page + 1}</h2>
                            {activeNextPage && <h2 className={`text-${theme}-500 flex items-center justify-center px-6 border-2 rounded-lg border-zinc-700 bg-gradient-to-tr from-zinc-900 via-zinc-800 to-zinc-900 shadow-md shadow-zinc-600 transition-all duration-500 ease-in-out animate-fadeIn`}>{page + 2}</h2>}
                        </div>

                        {activeNext && <button className={`text-${theme}-500 font-mono font-bold text-lg flex items-center justify-center backdrop-blur-lg bg-gradient-to-tr from-zinc-900 via-zinc-800 to-zinc-900 rounded-md py-2 px-6 shadow-md shadow-zinc-600 hover:shadow-glow hover:shadow-${theme}-500 hover:-translate-y-1 hover:scale-110 duration-700 w-28`} onClick={handleNext}>Next</button>}
                    </div>
                </ul>
            </div>
        </div>
