            ...
            <div className="w-full flex items-center justify-center mt-10">
                <ul className="w-9/12 space-y-4 bg-slate-900 rounded-t-xl flex flex-col justify-center items-center">
                    {slicedGames.map((game) => (
                        <Link to={`/game/${game.id}`} key={game.id} className="transition ease-in-out delay-150 hover:-translate-y-1 hover:scale-105 shadow-2xl bg-gradient-to-b from-zinc-900 to bg-zinc-900 via-zinc-800 border-r-4 border-emerald-500 p-4 w-11/12 flex rounded-l-xl space-x-3 mt-10">
                            <div>
