...
const SearchInput = ({ setIsMenuActive }: { setIsMenuActive: (isMenuActive: boolean) => void }) => {
    ...
    return (
        <form autoComplete="off" className={`relative -top-1 lg:block text-zinc-200  transition-opacity duration-500 ease-in-out animate-fadeIn`} onSubmit={handleSubmit}>
            <div className="relative">
                <input
                    placeholder="Search in catalogue..."
                    className={`shadow-glow-sm text-zinc-300 shadow-${theme}-500 bg-zinc-700 bg-opacity-30 border-2 border-transparent px-4 py-3 lg:px-3 lg:py-1 xl:px-4 xl:py-2 rounded-xl w-full lg:w-80 transition-all lg:focus:w-96 outline-none focus:border-${theme}-400 pr-12`}
                    name="search"
                    value={enteredText}
